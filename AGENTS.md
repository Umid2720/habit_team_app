...
Challenge participation and administrative authorization are separate. A user may be both an admin/sardor and a ranked participant, but participation never grants management permissions. Administrative mutations require explicit server-side role/permission checks; UI restrictions are insufficient. A participating admin may act normally as a participant and may manage challenge-wide settings, but must not review their own excuse or payment request, reduce their own penalty/debt, or make a historical correction that improves their own accountability or ranking. Those privileged actions require a different active challenge admin; with no such reviewer, the request/action remains pending or unresolved until another admin is added. Ordinary participants cannot perform historical corrections, penalty waivers, payment approvals, membership management, or habit-rule management.
